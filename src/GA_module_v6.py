...
#    Ipar = gen_parents(npop)
    ...
    if (nrvar > 0):
        Rnew = Rpop[np.argmin(PI[Ipar], axis=0)]
    if (nivar > 0):
        Inew = Ipop[np.argmin(PI[Ipar], axis=0)]
    if (nrvar > 0):
        Pnew = Ppop[np.argmin(PI[Ipar], axis=0)]
...
def int_rand_mutation(Iold, Ilimit, pmut):
    ...
    for i in range(npop):
        ...
        if p_mutation <= pmut:
            Imut[i] = I_min + \
                np.rint(np.floor(np.random.rand(nivar)*(I_max-I_min)))
        ...
        if pint < pxov:
            ...
            for j in range(1, npvar):
                ...
                if (len(inex) > 0):
                    ...
                    icity = inex[iran[0][math.floor(
                        len(iran)*np.random.random(1))]]
                else:
                    nnex = etab[np.where(etab[:][:] != -1)]
                    icity = nnex[math.floor(
                        len(nnex)*np.random.random(1))].astype(int)
                xoff[j] = icity
            Ppop[i][:] = xoff[:]
    # Return children
    return Ppop
...
def ga_min(PI_function: Callable[[np.ndarray, np.ndarray, np.ndarray], float], nvar, Ilimit, Rlimit, pvar, verbose=True, **kwargs):
    ...
    for iga in range(nga):
        ...
        if (nrvar > 0):
            Rpop = rea_rand_population(Rlimit, nrvar, npop)
        else:
            Rpop = np.zeros((npop, nrvar))
        if (nivar > 0):
            Ipop = int_rand_population(Ilimit, nivar, npop)
        else:
            Ipop = np.zeros((npop, nivar)).astype(int)
        if (npvar > 0):
            Ppop = per_rand_population(npvar, npop)
        else:
            Ppop = np.zeros((npop, npvar)).astype(int)

        if (nrvar > 0):
            Rbest = np.array([Rpop[0][:]])
        else:
            Rbest = np.zeros(0)
        if (nivar > 0):
            Ibest = np.array([Ipop[0][:]])
        else:
            Ibest = np.zeros(0).astype(int)
        if (npvar > 0):
            Pbest = np.array([Ppop[0][:]])
        else:
            Pbest = np.zeros(0).astype(int)
...
#        PI =  fitness_function(Rpop,Ipop,Ppop)
        ...
        for igen in range(ngen):
            ...
            Rpop, Ipop, Ppop = tournament_selection(
                Rpop, Ipop, Ppop, nrvar, nivar, npvar, PI, ptou)
...
#                print(ind,PI[ind])
                if (nrvar > 0):
                    Rbest = np.array(Rpop[ind[0][0]][:])
                if (nivar > 0):
                    Ibest = np.array(Ipop[ind[0][0]][:])
                if (npvar > 0):
